...
class BaseModel:
    ...
    def resume_training(self, resume_state):
        ...
        # Get the desired LR and WD from the current configuration (YAML)
        optim_g_opt = self.opt["train"]["optim_g"]
        new_lr = optim_g_opt["lr"]
        # Use .get() as a fallback, though it should be defined in your YAML
        new_wd = optim_g_opt.get("weight_decay", 0)

        for i, o in enumerate(resume_optimizers):
            self.optimizers[i].load_state_dict(
                o
            )  # Load the full state (including momentum/Adam buffers)

            # 1. Manually override LR and WD in the loaded parameter groups
